                ...
                <ul class="products-list product-list-in-card pl-2 pr-2">
                  ...
                  <li class="item">
                    <div class="product-img">
                      <img src="uploads/img/products/<?=$product->img?>" alt="<?=$product->img?>" class="img-size-50">
                    </div>
                ...
                <table id="" class="datatable table table-bordered table-striped">
                  <thead>
                        ...
                        <tbody id="event-today-list-tbody">
                         <?php
                         $get_event = $pdo->prepare("SELECT events.id,events.status, events.appt_code, customers.name, events.title,  DATE_FORMAT(events.start, '%d - %M %Y %h:%i %p') as start from events inner join customers on events.customer_id=customers.id where status in ('PENDING','FOR APPROVAL','DONE') and events.isPersonal='false' ORDER BY `start` desc LIMIT 5");
                         $get_event->execute();
