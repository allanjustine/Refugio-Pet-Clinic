      ...
      <script type="text/javascript">
        window.addEventListener("load", function() {
          swal({
            ...
            text: "<?php echo $_GET['del'] ?>, Has Been Deleted",
            icon: "info",
             buttons: {
               confirm: {
                text: "OK",
                    ...
                    <tbody>
                      ...
                      while ($customer = $get_customer->fetch(PDO::FETCH_OBJ)) {
                        ...
                        <tr>
                          ...
                          <td style="white-space:nowrap !important; width: 30px !important">
                            <button onclick="showEditModal(<?=$customer->id?>)" class="btn btn-sm btn-info shadow" style="border-radius: 5px"><span><i class="fas fa-edit"></i></span>&nbsp;&nbsp;Update</button>
                            <button onclick='delCustomer(<?=$customer->id?>,"<?=$customer->email?>","<?=$customer->name?>")' class="btn btn-sm btn-danger shadow" style="border-radius: 5px"><span><i class="fas fa-trash"></i></span>&nbsp;&nbsp;Delete</button>
                          </td>
                        </tr>
                        <?php  
                      }
                  ...
                  <tbody>
                    ...
                    while ($pet = $get_pet->fetch(PDO::FETCH_OBJ)) {
                      ...
                      <tr>
                        ...
                        <td style="white-space:nowrap !important; width: 30px !important">
                          <button onclick="showEditPet(<?=$pet->pet_id?>)" class="btn btn-sm btn-info shadow" style="border-radius: 5px"><span><i class="fas fa-edit"></i></span>&nbsp;&nbsp;Update</button>
...
<!-- /.modal -->
<script type="text/javascript">
  function delCustomer(id,email,name) {
    //toastr.info("test:"+id)
    swal({
      title: `Delete Customer?`,
      text: `Selected customer will be deleted.`,
      icon: "warning",
      buttons: {
       confirm: {
        text: "Continue",
        ...
        success: function (res){
          ...
          if (data.response=="exist"){
            swal({
              ...
              text: `Customers with existing records or verified can't be deleted.`,
              icon: "error",
              buttons: {
               confirm: {
                text: "Ok",
            ...
            location.href = "customers.php?del="+name
          }
