...
$query = "SELECT sum(order_details.qty) as count FROM `order_details` inner join orders on order_details.order_id=orders.id where orders.  customer_id=:id and orders.status='UNPAID' ";
...
$query = "SELECT COUNT(*) as count FROM `events` where customer_id=:id and status='PENDING' and isPersonal='false' ";
$stmt = $pdo->prepare($query);
$stmt->bindParam(':id',$_SESSION['id']);
$stmt->execute();
$result = $stmt->fetch();
...
$query = "SELECT sum(order_details.total) as sum FROM order_details inner join orders on order_details.order_id=orders.id WHERE orders.status='PAID' and orders.customer_id=:id;";
$stmt = $pdo->prepare($query);
$stmt->bindParam(':id',$_SESSION['id']);
$stmt->execute();
$result = $stmt->fetch();
...
$query = "SELECT sum(bill) as sum FROM services inner join events on services.event_id=events.id WHERE services.status='PAID' and events.customer_id=:id;";
$stmt = $pdo->prepare($query);
$stmt->bindParam(':id',$_SESSION['id']);
$stmt->execute();
$result = $stmt->fetch();
      ...
      <div class="row">
        ...
        <div class="col-12 col-sm-6 col-md-4">
          <div class="info-box mb-3">
            <span class="info-box-icon bg-danger elevation-1"><i class="fas fa-calendar-days"></i></span>
            ...
            <h3 class="card-title">MOST BOUGHT PRODUCTS</h3>
              ...
              <div class="card-body pt-0 pb-0">
                <ul class="products-list product-list-in-card pl-2 pr-2">
                  <?php
                      $get_products = $pdo->prepare("SELECT products.name, products.description,products.img, sum(order_details.qty) as count FROM `order_details` INNER join orders on order_details.order_id=orders.id INNER join products on products.id=order_details.product_id where orders.customer_id=:id GROUP by products.id order by count desc LIMIT 4;");
                      $get_products->bindParam(':id',$_SESSION['id']);
              ...
              <div class="card-footer text-center">
                <a href="shop.php" class="uppercase">View All Products</a>
              </div>
              <!-- /.card-footer -->
            </div>
            <!-- /.card -->
                ...
                <table id="" class="datatable table table-bordered table-striped">
                  <thead>
                        ...
                        <tbody id="event-today-list-tbody">
                         <?php
                         $get_event = $pdo->prepare("SELECT events.id,events.status, events.appt_code, events.title,  DATE_FORMAT(events.start, '%d - %M %Y %h:%i %p') as start from events inner join customers on events.customer_id=customers.id where customers.id=:id and events.isPersonal='false' ORDER BY `start` ASC LIMIT 5");
